...
function buildPasswordResetHtml(params: PasswordResetEmailParams): string {
  const { userName, gymName, resetCode } = params;
      ...
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                ...
                <tr>
                  <td align="center" style="padding:20px 0;">
                    <table cellpadding="0" cellspacing="0">
                      <tr>
                        ${digits.map((d) => `<td style="padding:0 4px;"><div style="width:48px; height:56px; background-color:#f3f4f6; border:2px solid #e5e7eb; border-radius:8px; text-align:center; line-height:56px; font-size:28px; font-weight:bold; color:#111827;">${d}</div></td>`).join("")}
                      </tr>
                    </table>
                  </td>
                </tr>
          ...
          <tr>
            <td style="background-color:#f9fafb; padding:20px 40px; text-align:center; border-top:1px solid #e5e7eb;">
              <p style="color:#9ca3af; font-size:12px; margin:0;">
                ${gymName} &mdash; Este es un email automático, no respondas a este mensaje.
              </p>
            </td>
          </tr>
        </table>
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    const info = await transporter.sendMail({
      ...
      to: params.toEmail,
      subject: `Código de recuperación - ${params.gymName}`,
